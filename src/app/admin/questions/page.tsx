   ...
   return (
      <div>
         ...
         <main className="flex flex-col items-center max-h-[calc(100dvh-100px)] overflow-y-auto">
            <h2 className="text-2xl font-bold text-center">Adicionar nova questão</h2>
            <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-3xl space-y-4 px-4 mt-2">
               ...
               {/* novo */}
               <select
                  {...register('type_question', { required: true })}
                  className="w-full p-3 border border-green-500 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500"
               >
            ...
            <div className="mt-6 w-full max-w-3xl mx-auto px-4 ">
               ...
               {questionsList.map((question) => (
                  <div key={question.id} className=" border rounded mb-4 p-4 bg-white">
                     ...
                     <p className="text-gray-600 mb-3">Tipo:{question.type_question || 'sem tipo'}</p>
